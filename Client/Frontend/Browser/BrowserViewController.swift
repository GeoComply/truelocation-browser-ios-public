...
extension BrowserViewController: TabDelegate {
    ...
    func tab(_ tab: Tab, didCreateWebView webView: WKWebView) {
        ...
        let geolocateHelper = GeolocateHelper(tab: tab)
        tab.addContentScriptToPage(geolocateHelper, name: GeolocateHelper.name())
        
        let geoguardHelper = GeoGuardHelper(tab: tab)
        tab.addContentScriptToPage(geoguardHelper, name: GeoGuardHelper.name())
...
extension BrowserViewController {
    ...
    func presentSignInViewController(_ fxaOptions: FxALaunchParams? = nil, flowType: FxAPageType = .emailLoginFlow, referringPage: ReferringPage = .none) {
        /*
        // GG Overriden
        // Comment FirefoxAccountSignInViewController
...
extension BrowserViewController: IntroViewControllerDelegate {
    func introViewControllerDidFinish(_ introViewController: IntroViewController, showLoginFlow: FxALoginFlow?) {
        self.profile.prefs.setInt(1, forKey: PrefsKeys.IntroSeen)
        introViewController.dismiss(animated: false) {
            if self.navigationController?.viewControllers.count ?? 0 > 1 {
                _ = self.navigationController?.popToRootViewController(animated: true)
            }
            
            if let _ = showLoginFlow {
                let fxaParams = FxALaunchParams(query: ["entrypoint": "firstrun"])
                self.presentSignInViewController(fxaParams, flowType: .emailLoginFlow, referringPage: .none)
            }
        }
        self.introViewController = nil // Weak var should auto release.
    }
}
